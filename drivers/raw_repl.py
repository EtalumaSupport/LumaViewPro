...
def enter_raw_repl(serial_port, soft_reset=True):
    ...
    for attempt in range(1, ENTER_RETRIES + 1):
        ...
        try:
            ...
            if not data or b'raw REPL' not in data:
                logger.warning(f'Raw REPL entry attempt {attempt} failed: {data!r:.100}')
...
def list_files(serial_port):
    ...
    if result is None:
        logger.info('os.listdir failed -- filesystem may be unavailable on this board')
        return []
...
def read_file(serial_port, filename, verify=True):
    ...
    if verify:
        ...
        if data2 is None:
            logger.warning(
                f'Verification read of {filename} failed -- using first read ({len(data)} bytes)'
            )
...
def write_file(serial_port, filename, data):
    ...
    file_size = len(data)
    logger.info(f'Writing {safe_name} ({file_size} bytes, SHA256={expected_hash[:16]}...)')
    ...
    for attempt in range(1, WRITE_VERIFY_RETRIES + 1):
        ...
        try:
            ...
            if 'OK:' in resp_text:
                ...
                logger.info(
                    f'Successfully wrote {safe_name} '
    ...
    try:
        while time.time() < deadline:
            try:
                ...
                logger.debug(
                    '[raw_repl  ] firmware probe attempt failed; retrying: %s: %s',
                    type(e).__name__,
                    e,
                )
            time.sleep(1.0)

        logger.error(f'Firmware not responding after {timeout}s. Board may need power cycle.')
        return None
    finally:
        serial_port.timeout = old_timeout
