...
class UpdateError(Exception):
    ...
    def __init__(self, message: str, stage: UpdateStage, recoverable: bool = True):
...
def _detect_bootsel_windows():
    ...
    try:
        ...
        for letter in 'DEFGHIJKLMNOPQRSTUVWXYZ':
            drive = f'{letter}:\\'
            result = kernel32.GetVolumeInformationW(drive, buf, 256, None, None, None, None, 0)
...
def _flash_uf2_picotool(uf2_path, picotool_path=None, reboot=True):
    ...
    try:
        ...
        result = subprocess.run(
            [picotool_path, 'load', str(uf2_path)], capture_output=True, text=True, timeout=60
        )
        ...
        if reboot:
            time.sleep(1.0)
            result = subprocess.run(
                [picotool_path, 'reboot'], capture_output=True, text=True, timeout=10
            )
            if result.returncode != 0:
...
def _wait_for_serial_port(vid, pid, timeout=30.0):
    ...
    logger.info(f'Waiting for serial port VID=0x{vid:04X} PID=0x{pid:04X} (timeout={timeout}s)...')
    deadline = time.time() + timeout
    while time.time() < deadline:
...
def _backup_configs(board, board_config, backup_dir, callback=None):
    ...
    _report_progress(
        callback,
        UpdateStage.BACKING_UP_CONFIG,
        f'Entering raw REPL on {board_config.label} board...',
        0.12,
    )
    ...
    try:
        ...
        for filename in board_config.config_files:
            ...
            _report_progress(
                callback, UpdateStage.BACKING_UP_CONFIG, f'Reading {filename}...', 0.14
            )
    ...
    try:
        ...
        for filename, data in config_data.items():
            if filename in board_files:
                ...
                if existing == data:
                    logger.info(f'{filename} survived update -- skipping restore')
                    continue
                else:
                    logger.warning(
                        f'{filename} exists but differs from backup -- restoring from backup'
                    )

            _report_progress(
                callback, UpdateStage.RESTORING_CONFIG, f'Restoring {filename}...', 0.85
            )

            if not board.repl_write_file(filename, data):
                raise UpdateError(
                    f'Failed to restore config: {filename}. Backup available on local disk.',
                    stage=UpdateStage.RESTORING_CONFIG,
                )
...
def update_firmware(
    ...
    if backup_dir is None:
        ...
        backup_dir = Path.home() / 'Documents' / 'Etaluma' / 'firmware_backups' / ts
    ...
    try:
        ...
        _report_progress(progress_callback, UpdateStage.PREFLIGHT, 'Checking UF2 file...', 0.0)
        ...
        _report_progress(
            progress_callback,
            UpdateStage.CHECKING_VERSION,
            f'Connecting to {config.label} board...',
            0.05,
        )
        ...
        if current_version == target_version and current_version is not None:
            ...
            _report_progress(
                progress_callback, UpdateStage.COMPLETE, 'Already at target version', 1.0
            )
            board.disconnect()
            return result
        ...
        if not skip_config_backup:
            _report_progress(
                progress_callback, UpdateStage.BACKING_UP_CONFIG, 'Backing up config files...', 0.10
            )
            config_data = _backup_configs(board, config, backup_dir, progress_callback)
        ...
        _report_progress(
            progress_callback, UpdateStage.SENDING_FWUPDATE, 'Sending FWUPDATE command...', 0.25
        )
        ...
        _report_progress(
            progress_callback, UpdateStage.WAITING_BOOTSEL, 'Waiting for BOOTSEL drive...', 0.30
        )
        bootsel_drive = _wait_for_bootsel_drive(timeout=config.bootsel_timeout)

        if bootsel_drive is not None:
            # ---- Stage 6: Copy UF2 file via mass storage ----
            _report_progress(
                progress_callback,
                UpdateStage.COPYING_UF2,
                f'Copying {uf2_path.name} to board...',
                0.40,
            )
            ...
            if picotool is not None:
                ok = _flash_uf2_picotool(uf2_path, picotool_path=picotool, reboot=True)
                if ok:
        ...
        _report_progress(
            progress_callback, UpdateStage.WAITING_REBOOT, 'Waiting for board to reboot...', 0.55
        )
        ...
        _report_progress(
            progress_callback,
            UpdateStage.VERIFYING_VERSION,
            'Verifying new firmware version...',
            0.65,
        )
        ...
        if new_version is None:
            result.warnings.append('Could not read firmware version after update')
        elif target_version and new_version != target_version:
            result.warnings.append(
                f'Version mismatch: expected {target_version}, got {new_version}'
            )

        # ---- Stage 9: Restore config files ----
        if config_data:
            _report_progress(
                progress_callback, UpdateStage.RESTORING_CONFIG, 'Restoring config files...', 0.75
            )
        ...
        if not skip_post_test:
            _report_progress(
                progress_callback, UpdateStage.POST_UPDATE_TEST, 'Running post-update test...', 0.90
            )
        ...
        board2.disconnect()
        result.success = True
        _report_progress(progress_callback, UpdateStage.COMPLETE, 'Firmware update complete', 1.0)
        ...
        _report_progress(progress_callback, UpdateStage.FAILED, f'Unexpected error: {e}', 0.0)
        return result
...
def nuke_board(
    ...
    try:
        ...
        _report_progress(
            ...
            f'Preparing to nuke {config.label} board...',
            0.0,
        )
        ...
        if bootsel_drive is not None:
            ...
            _report_progress(
                progress_callback, UpdateStage.SENDING_FWUPDATE, 'Entering BOOTSEL mode...', 0.10
            )
            ...
            _report_progress(
                progress_callback, UpdateStage.WAITING_BOOTSEL, 'Waiting for BOOTSEL drive...', 0.25
            )
            bootsel_drive = _wait_for_bootsel_drive(timeout=config.bootsel_timeout)
        ...
        if bootsel_drive is not None:
            ...
            if platform.system() == 'Darwin':
                _report_progress(
                    progress_callback,
                    UpdateStage.COPYING_UF2,
                    "Note: macOS may show 'disk not ejected properly' -- this is normal.",
                    0.40,
                )
            ...
            picotool = _find_picotool()
            if picotool is not None:
                ok = _flash_uf2_picotool(nuke_uf2_path, picotool_path=picotool, reboot=False)
                if ok:
        ...
        _report_progress(
            progress_callback,
            UpdateStage.WAITING_REBOOT,
            'Waiting for board to return to BOOTSEL...',
            0.70,
        )
        time.sleep(3.0)
        ...
        if bootsel_drive is not None:
            ...
            _report_progress(
                ...
                f'Flash erased. Board is in BOOTSEL mode at {bootsel_drive} -- ready for new UF2.',
                1.0,
            )
            ...
            _report_progress(
                progress_callback,
                UpdateStage.COMPLETE,
                'Flash erased. Replug with BOOTSEL held to flash new firmware.',
                1.0,
            )
...
def deploy_firmware_file(
    ...
    try:
        # ---- Stage 1: Pre-flight checks ----
        _report_progress(progress_callback, UpdateStage.PREFLIGHT, 'Checking firmware file...', 0.0)
        ...
        _report_progress(
            progress_callback, UpdateStage.RESTORING_CONFIG, 'Deploying firmware file...', 0.40
        )
        ...
        _report_progress(
            progress_callback, UpdateStage.VERIFYING_VERSION, 'Rebooting firmware...', 0.75
        )
        ...
        board.disconnect()
        result.success = True
        _report_progress(progress_callback, UpdateStage.COMPLETE, 'Firmware deploy complete', 1.0)
        logger.info(f'Firmware deploy successful: {current_version} -> {new_version}')
        return result
