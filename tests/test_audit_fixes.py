...
class TestAutoExposureBoundsOpenedAtAGEnable_655:
    """Issue #655 -- Red channel AG histogram dim.

    Pre-fix, pyloncamera.auto_gain() set GainAuto + ExposureAuto to
    Continuous without writing AutoExposureTimeLowerLimit /
    AutoExposureTimeUpperLimit. The Pylon driver kept whatever bounds
    were cached from the prior session -- typically a narrow band
    around the last-written ExposureTime. AG was implicitly capped
    above by that ceiling, so Red fluorescence layers with low signal
    hit max_gain_db (20 dB) and had nowhere to go on the exposure
    axis. Chris's 2026-05-22 TIFFs confirm: gain=20 dB cap, exposure
    pinned at the layer default 100 ms across all 3 channels; Red
    mean grey value 3.7 vs target ~76.

    Fix: _open_auto_exposure_time_bounds_to_camera_max() writes
    [LowerLimit.Min, UpperLimit.Max] from the sensor's native range
    when AG is enabled (Continuous or Once). Live + protocol AG both
    call the helper. Cluster note: pairs with #673 fix
    (deadline-at-arm-time) -- together AG now has both convergence
    TIME (#673) and convergence ROOM on the exposure axis (#655).
    """
    ...
    def test_helper_method_exists(self):
        """Helper must exist at class scope on PylonCamera."""
        src = self._src()
        assert 'def _open_auto_exposure_time_bounds_to_camera_max(' in src, (
            'PylonCamera must expose _open_auto_exposure_time_bounds_to_camera_max() (#655)'
        )

    def test_helper_writes_both_lower_and_upper_limits(self):
        """Helper body must SetValue both AutoExposureTime bounds
        from the sensor's native range (not hardcoded values)."""
        src = self._src()
        start = src.find('def _open_auto_exposure_time_bounds_to_camera_max(')
        assert start != -1
        body = src[start : start + 2000]
        end = body.find('\n    def ', 1)
        if end != -1:
            body = body[:end]
        assert 'AutoExposureTimeLowerLimit.SetValue(' in body, (
            'helper must SetValue AutoExposureTimeLowerLimit (#655)'
        )
        assert 'AutoExposureTimeUpperLimit.SetValue(' in body, (
            'helper must SetValue AutoExposureTimeUpperLimit (#655)'
        )
        assert 'AutoExposureTimeLowerLimit.Min' in body, (
            'helper must read .Min from the sensor (not hardcode) '
            'so different camera models get the right range (#655)'
        )
        assert 'AutoExposureTimeUpperLimit.Max' in body, (
            'helper must read .Max from the sensor (#655)'
        )

    def test_auto_gain_continuous_calls_helper(self):
        """The continuous AG entry point (auto_gain()) must open the
        exposure bounds before setting GainAuto/ExposureAuto =
        Continuous. Without this call AG cannot raise exposure when
        gain caps."""
        src = self._src()
        start = src.find('def auto_gain(')
        assert start != -1
        body = src[start : start + 3000]
        end = body.find('\n    def ', 1)
        if end != -1:
            body = body[:end]
        assert 'self._open_auto_exposure_time_bounds_to_camera_max()' in body, (
            'auto_gain() Continuous branch must call '
            '_open_auto_exposure_time_bounds_to_camera_max() so AG '
            'has exposure headroom (#655)'
        )

    def test_auto_gain_once_calls_helper(self):
        """The one-shot AG entry point (auto_gain_once()) must also
        open the bounds. Symmetric with auto_gain()."""
        src = self._src()
        start = src.find('def auto_gain_once(')
        assert start != -1
        body = src[start : start + 2000]
        end = body.find('\n    def ', 1)
        if end != -1:
            body = body[:end]
        assert 'self._open_auto_exposure_time_bounds_to_camera_max()' in body, (
            'auto_gain_once() Once branch must call '
            '_open_auto_exposure_time_bounds_to_camera_max() so the '
            'one-shot adjustment has exposure headroom (#655)'
        )
...
class TestProtocolPostProcessorBypassGate_672:
    """Issue #672 second-order fix: the base-class load_folder gate
    must honor the subclass write-bypass contract introduced by
    `9aea524`. A subclass that already wrote the output file itself
    (e.g. CompositeGeneration's RGB-native tifffile.imwrite path)
    signals "skip the base-class write" by returning image=None.

    Pre-fix the gate used key-presence (`'image' in alg_results`),
    which always evaluated True since subclasses always set the key.
    That routed None through cv2.imwrite and raised
    cv2.error: !_img.empty() on the FILE_WORKER thread, which the
    notification system surfaced as a generic "FILE task failed"
    popup AFTER the composite TIFF had already been written by the
    subclass. Chris's bench evidence captured that exact shape on
    2026-05-22 (SNlogs-2026-05-22-074501.zip).

    Fix: value-presence test (`alg_results.get('image') is not None`).
    Source pin + behavioral test guard the bypass contract for
    future subclass migrations (stitcher / stack / video).
    """

    def test_source_uses_value_presence_test(self):
        """AST pin: load_folder must use value-presence on the bypass
        signal, not key-presence. Catches a revert."""
        import ast
        import pathlib
        ...
        load_folder_body = None
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name == 'load_folder':
                load_folder_body = ast.unparse(node)
                break
        assert load_folder_body is not None, 'load_folder not found'
        assert (
            "alg_results.get('image') is not None" in load_folder_body
            or 'alg_results.get("image") is not None' in load_folder_body
        ), (
            'load_folder must use value-presence test on alg_results '
            'image to honor the subclass write-bypass contract '
            '(issue #672)'
        )
        assert (
            "'image' in alg_results" not in load_folder_body
            and '"image" in alg_results' not in load_folder_body
        ), (
            'load_folder must NOT use key-presence test on alg_results '
            'image -- subclasses always set the key, so key-presence '
            'fires cv2.imwrite(None) and raises (issue #672)'
        )

    def test_load_folder_skips_cv2_imwrite_when_subclass_returns_image_none(self):
        """Behavioral: when _group_algorithm returns image=None, the
        base-class load_folder must NOT call cv2.imwrite."""
        import pathlib
        import tempfile
        ...
        class _BypassSignaller(ProtocolPostProcessor):
            """Mimics CompositeGeneration's bypass: subclass wrote the
            file itself; tell the base class to skip cv2.imwrite."""
            ...
            def _group_algorithm(self, path, df, popup=None, **kwargs):
                return {
                    'status': True,
                    'image': None,
                    'metadata': {},
            ...
            @staticmethod
            def _add_record(*args, **kwargs):
                return

        instance = _BypassSignaller(
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
                ...
                [
                    {
                        'Filepath': 'a.tiff',
                        'Color': 'Red',
                        **{f: False for f in PostFunction.list_values()},
                    },
                    {
                        'Filepath': 'b.tiff',
                        'Color': 'Green',
            ...
            fake_record.file_exists_in_records.return_value = False
            helper_result = {
                'status': True,
                'images_df': df,
                'root_path': tmp,
                'protocol_post_record': fake_record,
            }
            instance._post_processing_helper.load_folder = MagicMock(
                return_value=helper_result,
            )
            instance._post_processing_helper.generate_output_dir_name = MagicMock(
                return_value='out_dir',
            )

            with patch('modules.protocol_post_processor.cv2.imwrite') as mock_imwrite:
                result = instance.load_folder(
                    path=str(tmp),
                    tiling_configs_file_loc=None,
                )

            assert mock_imwrite.call_count == 0, (
                'When _group_algorithm returns image=None, the base-'
                'class load_folder gate must skip cv2.imwrite. Got '
                f'{mock_imwrite.call_count} call(s). Issue #672.'
            )
            assert result['status'] is True, (
                f'load_folder must succeed when bypass is honored; got {result}'
            )
