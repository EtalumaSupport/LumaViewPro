...
def _artifact(*, scenario, machine, git_sha, config, functions):
    ...
    return {
        ...
        'functions': [
            {'function': n, 'cpu_cores': c, 'err_cores_95': e} for n, c, e in functions
        ],
    }
...
class TestCompatibility:
    def test_same_scenario_machine_config_different_sha_is_comparable(self):
        ...
        # matches -> comparable.
        before = _artifact(scenario='liveview-fit', machine='bench', git_sha='bebf28be',
                           config=_CFG, functions=[('f', 1.0, 0.02)])
        after = _artifact(scenario='liveview-fit', machine='bench', git_sha='91a5abd4',
                          config=_CFG, functions=[('f', 1.0, 0.02)])
        assert compatibility_issues(before, after) == []

    def test_config_mismatch_blocks(self):
        before = _artifact(scenario='liveview-fit', machine='bench', git_sha='a',
                           config={'live_view_fps': 30, 'preview_host_downscale': False},
                           functions=[('f', 1.0, 0.02)])
        after = _artifact(scenario='liveview-fit', machine='bench', git_sha='b',
    ...
    def test_machine_mismatch_blocks(self):
        before = _artifact(scenario='liveview-fit', machine='bench-A', git_sha='a',
                           config=_CFG, functions=[('f', 1.0, 0.02)])
        after = _artifact(scenario='liveview-fit', machine='bench-B', git_sha='b',
                          config=_CFG, functions=[('f', 1.0, 0.02)])
...
class TestCompare:
    def test_detects_vanished_resize_as_significant_improvement(self):
        ...
        before = _artifact(scenario='liveview-fit', machine='bench', git_sha='bebf28be', config=_CFG,
                           functions=[('resize (imgproc)', 0.85, 0.03), ('render', 0.40, 0.02)])
        after = _artifact(scenario='liveview-fit', machine='bench', git_sha='91a5abd4', config=_CFG,
    ...
    def test_flags_regression_beyond_noise(self):
        before = _artifact(scenario='s', machine='m', git_sha='a', config=_CFG,
                           functions=[('f', 0.20, 0.01)])
        after = _artifact(scenario='s', machine='m', git_sha='b', config=_CFG,
                          functions=[('f', 0.50, 0.01)])
        d = compare(before, after)[0]
        assert d.delta_cores > 0 and d.significant

    def test_small_change_within_noise_not_significant(self):
        before = _artifact(scenario='s', machine='m', git_sha='a', config=_CFG,
                           functions=[('f', 0.500, 0.05)])
        after = _artifact(scenario='s', machine='m', git_sha='b', config=_CFG,
                          functions=[('f', 0.510, 0.05)])
        d = compare(before, after)[0]
