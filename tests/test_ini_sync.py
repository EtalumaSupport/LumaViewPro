...
FIRMWARE_SRC_DIR = FIRMWARE_REPO / 'Motor Controller' / 'Firmware' if FIRMWARE_REPO else None
...
def _require_firmware_repo():
    ...
    if FIRMWARE_REPO is None:
        pytest.skip('Firmware repo not found at any expected location; skipping INI sync checks')
...
def test_ini_file_matches_firmware_repo(filename):
    ...
    assert src_bytes == dst_bytes, (
        f"{filename} differs between Firmware repo and LVP data/firmware/. Run: cp '{src}' '{dst}'"
    )
