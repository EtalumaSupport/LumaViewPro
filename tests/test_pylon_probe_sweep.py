...
class TestResolveResolutions:
    ...
    def test_mixed_tokens_keep_order(self):
        result = _resolve_resolutions(
            ['1900', '2100', 'sensor-max'], 4096, 3000)
...
class TestUsb3CellMatrix:
    def test_default_one_cell(self):
        ...
        assert cells == [
            {'pixel_format': 'Mono8', 'resolution': (2100, 2100),
    ...
    def test_dltl_on_with_values_expands(self):
        cells = _build_usb3_cells(
            _make_args(dltl_modes=['On'], dltl_values_mb=[160, 250, 360]))
        assert len(cells) == 3
...
class TestGigeCellMatrix:
    def test_default_one_cell(self):
        ...
        assert cells == [
            {'pixel_format': 'Mono8', 'resolution': (2100, 2100),
             'bw_mode': 'Performance', 'packet_size': 1500, 'delay_ticks': 0}
        ]
...
class TestFormatCellId:
    def test_usb3_dltl_off(self):
        cell = {'pixel_format': 'Mono8', 'resolution': (2100, 2100),
    ...
    def test_usb3_dltl_on_with_value(self):
        cell = {'pixel_format': 'Mono8', 'resolution': (1900, 1900),
                'dltl_mode': 'On', 'dltl_value': 160_000_000}
        out = _format_cell_id('usb3', cell)
        assert 'dltl=160M' in out

    def test_gige(self):
        cell = {'pixel_format': 'Mono8', 'resolution': (2100, 2100),
                'bw_mode': 'Performance', 'packet_size': 9000,
                'delay_ticks': 50}
