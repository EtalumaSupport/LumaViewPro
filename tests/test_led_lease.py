...
def test_release_frees_lease_for_next_owner(scope):
    ...
    second = scope.illumination.acquire_led_lease('autofocus', claim=held_run_claim())
    assert second is not None
    assert scope.illumination.led_lease_purpose == 'autofocus'


def test_child_lease_acquired_only_by_holder(scope):
    parent = scope.illumination.acquire_led_lease('protocol', claim=held_run_claim())
...
def test_dead_owner_lease_is_reclaimed_by_next_acquire(scope):
    ...
    # holder's claim no longer being held is the evidence that lets the
    # next acquire reclaim the stack instead of being refused.
    holder_claim = held_run_claim()
    wedged = scope.illumination.acquire_led_lease('protocol', claim=holder_claim)
    assert wedged is not None
    holder_claim.release()  # the owning run died without releasing its lease

    nxt = scope.illumination.acquire_led_lease('next', claim=held_run_claim())
...
def test_context_manager_acquires_and_releases(scope):
    with scope.illumination.acquire_led_lease('protocol', claim=held_run_claim()) as lease:
        assert lease.held
        assert scope.illumination.led_lease_purpose == 'protocol'
    assert scope.illumination.led_lease_purpose is None
...
def test_lease_violation_detects_external_writer(scope):
    ...
    stale = scope.illumination.acquire_led_lease('autofocus', claim=held_run_claim())
    stale.release()
